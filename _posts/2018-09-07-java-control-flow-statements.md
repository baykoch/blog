...
Java doğal yapısında olarak *döngü ifadeleri* ve *karar verme ifadeleri* vardı. Bunların yanında `break` gibi *dallanma ifadeleri* bulunur.
...
`for` ifadeleri diğer döngü ifadelerin tek yönde kontrol yapısı aksine başlangıç ve bitişle sınırlamaya zorlayan yapılardır.
...
3. `for` kontroll yapısında bileşik  `compound` ifadeler kullanılabilir. Örnekte biri artarak `i` biri azalarak `j` toplamı 10 eden sayılar.
...
`if` bir ifadeye bağlı karar veren, `if-else`yapısı ile dallanma yapabilen yapılardır.
...
if (condition) {        1.İlk şart
 ...
 .                        .
 .                        .
}else {}                N. Yukarıdaki hiçbir şart sağlanmazsa (zorunlu değil)
```

**Örnek**: `random` fonksiyonun  0 ile 100 aralığında hangi çeyreğe denk geldiğini bulan kod yazalım.
...
`Switch` yapısı `if - else if` ifadelerinden daha toplu ve hızlı olan yapılarıdır. Kontrol yapısında değere göre dallanma yapılır. 'case' kısımlarında **sabit** olmalıdır. `switch` yapısının desteklediği tipler: *byte, short, char, and int , enum , string sınıfı ve ilkel tiperin sarmanmış -wrap- sınıfları (Character, Byte, Short, and Integer)*
...
- `break` yazılmaz ise `break` buluncaya kadar çalışır.
...
- `default` hiçbir durum çalışmadığında çalışan kısımdır. Yeri önemli değildir; başta, ortada veya sonda bulunabilir. Son olarak zorunlu değildir.
...
`break` bulunduğu bloğun çalışmasını sonlandırırken, `continue` ise sade bulunduğu adımın çalışmasını sonlandırır.
...
`break`işaretlemiş  blogun kırılmasına  sağlar. Aşağıdaki kod parçasında birden fazla `3` olmasına rağmen ilk bulunduğu zaman aramayı bırakacaktır. `f:` kaldırıp sadece `break` kalsaydı bulmaya devam edecekti.
...
f: for (int i = 0; i < dizi.length; i++) {  // İlk sayı bulunduğunda kırılacak.
    ...
    for (int j = 0; j < 2; j++) {
        ...
        if ( dizi[i][j] == find ) {
            System.out.println((i) + ".satır " + (j) + ".sütunda bulundu!");
...
Yukarıdaki kod parçasında `break` yerine `continue` yazılsaydı, sadece bulduğu satırı atlayacaktı. Yani,  ilk değeri 2. satırda `{ 3, 3 }` bulacak 2.satırın 2. sütününe bakmadan diğer `{ 4, 3 }` satırına atlayacaktı.

### return

`return` bir metodun çağrıldığı yere dönmeyi sağlar.  Metot türüne göre bir değer geri döndürebilir veya döndürmez. Eğer bir değer geri döndermiyorsa kullanılmasına zorunlu olmayıp isteğe bağlı sade halinde `return;` kullanılabilir.
